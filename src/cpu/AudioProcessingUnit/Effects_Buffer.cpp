...
#include "AudioProcessingUnit/Effects_Buffer.h"
...
#include "AudioProcessingUnit/blargg_source.h"
...
void Effects_Buffer::apply_config() {
	...
	if (chans[0].vol[0] != TO_FIXED(1) || chans[0].vol[1] != TO_FIXED(0) || chans[1].vol[0] != TO_FIXED(0) ||
...
void Effects_Buffer::assign_buffers() {
	...
	for (int i = 0; i < (int)chans.size(); i++) {
		...
		int b = 0;
		for (; b < buf_count; b++) {
			if (ch.vol[0] == bufs[b].vol[0] && ch.vol[1] == bufs[b].vol[1] &&
...
#define CALC_LEVELS(vols, sum, diff, surround)                                                                         \
	...
	{                                                                                                                  \
		...
		if (vol_0 < 0)                                                                                                 \
			vol_0 = -vol_0, surround = true;                                                                           \
		fixed_t vol_1 = vols[1];                                                                                       \
		if (vol_1 < 0)                                                                                                 \
			vol_1 = -vol_1, surround = true;                                                                           \
		sum = vol_0 + vol_1;                                                                                           \
		diff = vol_0 - vol_1;                                                                                          \
	}
		...
		if (echo_phase && !no_echo) {
			...
			do {
				...
				do {
					...
					if (pos < out_pos)
						pos = out_pos;
					int count = blargg_ulong((char *)echo_end - (char const *)pos) / unsigned(stereo * sizeof(fixed_t));
					if (count > remain)
						count = remain;
					remain -= count;
