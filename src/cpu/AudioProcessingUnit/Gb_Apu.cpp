...
#include "AudioProcessingUnit/Blip_Buffer.h"
...
void Gb_Apu::synth_volume(int iv) {
	double div = static_cast<double>(osc_count) * 15 /*steps*/ * 8 /*master vol range*/ / iv;
	double v = volume_ * 0.60 / div;
	good_synth.volume(v);
	med_synth.volume(v);
}
...
int Gb_Apu::read_register(blip_time_t time, unsigned addr) {
	...
	static byte const masks[] = {0x80, 0x3F, 0x00, 0xFF, 0xBF, 0xFF, 0x3F, 0x00, 0xFF, 0xBF, 0x7F,
	                             0xFF, 0x9F, 0xFF, 0xBF, 0xFF, 0xFF, 0x00, 0x00, 0xBF, 0x00, 0x00,
