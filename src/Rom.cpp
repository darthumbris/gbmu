...
uint8_t Rom::read_u8(uint16_t addr) {
	...
	case 0xA000 ... 0xBFFF:
		// std::cout << "ram bank: " << (uint16_t)ram_bank << " wram_bank: " << (uint16_t)wram_bank_select() <<
		// std::endl; return ram_banks[ram_bank][addr - 0xA000]; //TODO make sure ext_ram and ram_bank are correct()
