...
void MCB3::write_u8(uint16_t addr, uint8_t val) {
	...
	case 0x000 ... 0x7FFF:
		if (addr <= 0x1FFF) {
            ...
            // TODO latched stuff (when writing 0x00 then 0x01 to this, current time becomes latched to the RTC register)
            // the latched data will not change until it becomes latched again by repeating the 0x00 -> 0x01 procedure
        }
		break;
