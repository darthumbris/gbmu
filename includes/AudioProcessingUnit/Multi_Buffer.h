...
public:
	...
	virtual channel_t channel(int index) BLARGG_PURE(;)
	    ...
	    virtual blargg_err_t set_sample_rate(long rate, int msec = blip_default_length)
	        BLARGG_PURE(;) virtual void clock_rate(long) BLARGG_PURE({}) virtual void bass_freq(int)
	            BLARGG_PURE({}) virtual void clear() BLARGG_PURE({}) long sample_rate() const;
	...
	// See Blip_Buffer.h
	virtual long read_samples(blip_sample_t *, long) BLARGG_PURE({ return 0; }) virtual long samples_avail() const
